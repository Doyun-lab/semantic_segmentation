...
Original file is located at
    https://colab.research.google.com/drive/1qM1yrCYAixHnbKb_GT9WTJTLCBwHkVxG
"""
...
# 사용 Model : PSPNet, FPN, LinkNet, PSPNet
# 사용 Backbone : vgg16,resnet50, resnet101, resnet152, resnext101, seresnext101, inceptionresnetv2, mobilenetv2
...
# 모델 불러오기
# 원하는 Model을 설정하고 backbone_name에 원하는 Backbone을 작성하여 사용
base_model = Unet(backbone_name='resnet152', encoder_weights='imagenet', encoder_freeze=True, activation="sigmoid", classes =3)
model = base_model

# Model Compile
model.compile(optimizer=tf.keras.optimizers.SGD(learning_rate=0.1, momentum=0.9, nesterov=True), loss="MSE", metrics=[miou.mean_iou, iou_score])
...
for i in range(0,np.shape(pred)[0]):
    ...
    # 원하는 경로에 그림 저장
    plt.savefig('/content/gdrive/MyDrive/Unet_resnet152/' + '%03d' % int(i) + '_pred.png')
